export const INTERVIEW_PREP_PROGRESS_KEY = "de-prep-hub-interview-prep-progress";
...
const DEFAULT_PROGRESS: InterviewPrepProgress = {
  practiceSolved: [],
  interviewConfidence: {},
  notesChaptersRead: [],
  visitDates: [],
...
export function getInterviewPrepProgress(): InterviewPrepProgress {
  if (typeof window === "undefined") return DEFAULT_PROGRESS;

  try {
    const stored = localStorage.getItem(INTERVIEW_PREP_PROGRESS_KEY);
...
function saveProgress(progress: InterviewPrepProgress): void {
  localStorage.setItem(INTERVIEW_PREP_PROGRESS_KEY, JSON.stringify(progress));
  window.dispatchEvent(new CustomEvent("interview-prep-progress-updated"));
}

export function recordInterviewPrepVisit(path: string, label: string): void {
  const progress = getInterviewPrepProgress();
...
export function isPracticeSolved(id: number): boolean {
  return getInterviewPrepProgress().practiceSolved.includes(id);
}
...
export function getInterviewConfidence(id: number): InterviewConfidence | null {
  return getInterviewPrepProgress().interviewConfidence[String(id)] ?? null;
}
...
export function getInterviewPrepStreak(): number {
  const dates = [...getInterviewPrepProgress().visitDates].sort().reverse();
...
export function getPracticeStats(total: number) {
  const solved = getInterviewPrepProgress().practiceSolved.length;
  return {
    solved,
    total,
...
export function getInterviewStats(total: number) {
  const entries = Object.values(getInterviewPrepProgress().interviewConfidence);
...
export function getNotesStats(totalChapters: number) {
  const read = getInterviewPrepProgress().notesChaptersRead.length;
  return { read, total: totalChapters, percent: totalChapters > 0 ? Math.round((read / totalChapters) * 100) : 0 };
}

export function exportInterviewPrepProgress(): string {
  return JSON.stringify(
    {
      version: 1,
      exportedAt: new Date().toISOString(),
      progress: getInterviewPrepProgress(),
    },
    null,
    2
  );
...
export function importInterviewPrepProgress(raw: string): { ok: boolean; message: string } {
  try {
    const payload = JSON.parse(raw);
    if (!payload.progress) {
      return { ok: false, message: "Invalid backup file — missing progress data." };
    }
    localStorage.setItem(INTERVIEW_PREP_PROGRESS_KEY, JSON.stringify(payload.progress));
    window.dispatchEvent(new CustomEvent("interview-prep-progress-updated"));
