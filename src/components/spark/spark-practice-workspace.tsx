...
import { pickRandomQuestions, QUIZ_SIZE } from "@/lib/spark-practice";
...
type TierFilter = "all" | SparkPracticeQuestion["tier"];
type CategoryFilter = "all" | SparkPracticeQuestion["category"];
type ViewMode = "browse" | "quiz";

const categoryLabels: Record<CategoryFilter, string> = {
  all: "All",
  ...SPARK_PRACTICE_CATEGORIES,
};
...
interface SparkPracticeWorkspaceProps {
  questions: SparkPracticeQuestion[];
  syncSelectionToUrl?: boolean;
}

export function SparkPracticeWorkspace({ questions, syncSelectionToUrl }: SparkPracticeWorkspaceProps) {
  ...
  const [selectedId, setSelectedId] = usePracticeQuestionId(questions, undefined, {
    syncToUrl: syncSelectionToUrl,
  });
  const [quizQuestions, setQuizQuestions] = React.useState<SparkPracticeQuestion[]>([]);
  ...
  React.useEffect(() => {
    refreshSolved();
    window.addEventListener("spark-progress-updated", refreshSolved);
    return () => window.removeEventListener("spark-progress-updated", refreshSolved);
  }, [refreshSolved]);
          ...
          <section id="practice-editor" className="panel order-1 p-5 sm:p-6 lg:order-2 lg:sticky lg:top-12 lg:self-start lg:max-h-[calc(100dvh-3rem)] lg:overflow-y-auto lg:overscroll-contain">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-muted/80 px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                {SPARK_PRACTICE_CATEGORIES[activeQuestion.category]}
              </span>
            </div>
            ...
            <div className="mt-5">
              <SparkEditor value={userCode} onChange={setUserCode} height={220} />
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={markSolved}
                className={cn(buttonVariants({ size: "sm" }), "bg-orange-600 hover:bg-orange-600/90")}
              >
                Mark solved
              </button>
            </div>
            ...
            {showSolution && (
              <div className="mt-4">
                <SparkCodeBlock code={activeQuestion.solution} />
              </div>
            )}
