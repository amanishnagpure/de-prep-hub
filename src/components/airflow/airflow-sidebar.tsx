    ...
    <>
      ...
      >
        <div className="sticky top-24 flex h-[calc(100vh-6rem)] flex-col p-4">
          <div className={cn("mb-6 flex items-center gap-3", collapsed && "justify-center")}>
            <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500/20 to-green-500/10 text-emerald-400 ring-1 ring-emerald-500/20">
              <Workflow className="size-5" />
            </div>
            {!collapsed && (
              <div>
                <p className="text-xs text-emerald-500">Orchestrate pipelines.</p>
              </div>
            )}
          </div>
      ...
      <nav className="flex gap-2 overflow-x-auto border-b border-border/70 px-4 py-3 lg:hidden">
        ...
        <Link
          ...
          className={cn(
            "shrink-0 rounded-full px-3 py-2 text-sm font-medium",
            pathname.startsWith(AIRFLOW_ROUTES.practice)
              ? "bg-emerald-500/10 text-emerald-500"
              : "bg-muted/50 text-muted-foreground"
          )}
        >
        ...
        <Link
          ...
          className={cn(
            "shrink-0 rounded-full px-3 py-2 text-sm font-medium",
            pathname.startsWith(AIRFLOW_ROUTES.interview)
              ? "bg-emerald-500/10 text-emerald-500"
              : "bg-muted/50 text-muted-foreground"
          )}
        >
