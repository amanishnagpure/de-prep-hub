...
import { AirflowEditor } from "@/components/airflow/airflow-editor";
import { AirflowCodeBlock } from "@/components/airflow/airflow-code-block";
import { buttonVariants } from "@/components/ui/button";
import { cn } from "cn";
import { usePracticeScrollToEditor } from "@/hooks/use-practice-scroll";

type TierFilter = "all" | AirflowPracticeQuestion["tier"];
type CategoryFilter = "all" | AirflowPracticeQuestion["category"];
type ViewMode = "browse" | "quiz";

const categoryLabels: Record<CategoryFilter, string> = {
  all: "All",
  ...AIRFLOW_PRACTICE_CATEGORIES,
};
...
export function AirflowPracticeWorkspace({ questions }: AirflowPracticeWorkspaceProps) {
  ...
  const [quizQuestions, setQuizQuestions] = React.useState<AirflowPracticeQuestion[]>([]);
  ...
  React.useEffect(() => {
    refreshSolved();
    window.addEventListener("airflow-progress-updated", refreshSolved);
    return () => window.removeEventListener("airflow-progress-updated", refreshSolved);
  }, [refreshSolved]);
          ...
          <aside className="order-2 space-y-3 lg:order-1 lg:sticky lg:top-14 lg:max-h-[calc(100dvh-3.5rem)] lg:overflow-y-auto lg:overscroll-contain">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search DAG problems..."
                className="w-full rounded-xl border border-border bg-background/60 py-2.5 pl-10 pr-3 text-sm outline-none ring-emerald-500/30 focus:ring-2"
              />
            </div>
              ...
              {(Object.keys(tierLabels) as TierFilter[]).map((key) => (
                <button
                  ...
                  className={cn(
                    ...
                    tier === key
                      ? "bg-emerald-500/10 text-primary ring-1 ring-emerald-500/20"
          ...
          <section id="practice-editor" className="panel order-1 p-5 sm:p-6 lg:order-2 lg:sticky lg:top-14 lg:self-start lg:max-h-[calc(100dvh-3.5rem)] lg:overflow-y-auto lg:overscroll-contain">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-muted/80 px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                {AIRFLOW_PRACTICE_CATEGORIES[activeQuestion.category]}
              </span>
            </div>
            ...
            <div className="mt-5">
              <AirflowEditor value={userCode} onChange={setUserCode} height={280} />
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={markSolved}
                className={cn(buttonVariants({ size: "sm" }), "bg-emerald-600 hover:bg-emerald-600/90")}
              >
                Mark solved
              </button>
            </div>
            ...
            {showSolution && (
              <div className="mt-4">
                <AirflowCodeBlock code={activeQuestion.solution} />
              </div>
            )}
