...
import type { PythonNotesChapter } from "@/lib/python-notes";
import { PYTHON_NOTES_SECTIONS } from "@/lib/python";
import { extractHeadings } from "@/lib/markdown-utils";
import { markNotesChapterRead, getPythonProgress } from "@/lib/python-progress";
...
interface PythonNotesViewProps {
  chapters: PythonNotesChapter[];
}

const PRIMARY_CHAPTER_IDS = new Set<string>(PYTHON_NOTES_SECTIONS.map((section) => section.id));

export function PythonNotesView({ chapters }: PythonNotesViewProps) {
  ...
  React.useEffect(() => {
    const refresh = () => setReadChapters(getPythonProgress().notesChaptersRead);
    refresh();
  ...
  return (
    <div className="space-y-6">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-muted font-medium text-foreground ring-1 ring-amber-500/25"
      ...
      <LabNotesChapterPanel
        ...
        accent="amber"
        Markdown={PythonMarkdownRenderer}
