...
const NAV: NavGroup[] = [
  {
    ...
    match: (pathname) => pathname.startsWith(PYTHON_ROUTES.notes),
    children: PYTHON_NOTES_SECTIONS.map((section) => ({
      label: section.label,
      href: `${PYTHON_ROUTES.notes}?chapter=${section.id}`,
      match: (pathname, chapter) =>
        pathname.startsWith(PYTHON_ROUTES.notes) &&
  ...
  },
  {
    ...
    match: (pathname) => pathname.startsWith(PYTHON_ROUTES.interview),
  },
  {
    id: "overview",
    label: "Overview",
    ...
    match: (pathname) => pathname === PYTHON_ROUTES.home,
  },
];
...
export function PythonSidebar() {
  ...
  return (
    <LabSidebarShell
      ...
      mobileNav={mobileNav}
      header={<p className="text-xs font-medium text-muted-foreground">Python</p>}
    >
      {NAV.map((group) => {
