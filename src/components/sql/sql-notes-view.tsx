...
import { SQL_NOTES_SECTIONS } from "@/lib/sql";
import { extractHeadings } from "@/lib/markdown-utils";
import { markNotesChapterRead, getSqlProgress } from "@/lib/sql-progress";
import { SqlMarkdownRenderer } from "@/components/sql/sql-markdown-renderer";
import { SqlScrollToc } from "@/components/sql/sql-scroll-toc";
import { LabNotesChapterPanel } from "@/components/notes/lab-notes-chapter-panel";
import { buttonVariants } from "@/components/ui/button";
import { cn } from "cn";

interface SqlNotesViewProps {
  chapters: SqlNotesChapter[];
}

const PRIMARY_CHAPTER_IDS = new Set<string>(SQL_NOTES_SECTIONS.map((section) => section.id));

export function SqlNotesView({ chapters }: SqlNotesViewProps) {
  ...
  React.useEffect(() => {
    const refresh = () => setReadChapters(getSqlProgress().notesChaptersRead);
    refresh();
  ...
  return (
    <div className="space-y-6">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-cyan-500/10 text-cyan-500 ring-1 ring-cyan-500/25"
      ...
      <LabNotesChapterPanel
        ...
        accent="cyan"
        Markdown={SqlMarkdownRenderer}
        ScrollToc={SqlScrollToc}
        headings={headings}
        onMarkRead={markRead}
      />
    </div>
