...
interface DatabricksCodeBlockProps {
  code: string;
  language?: string;
  showLineNumbers?: boolean;
}

export function DatabricksCodeBlock({
  code,
  language = "sql",
  showLineNumbers = true,
}: DatabricksCodeBlockProps) {
  ...
  return (
    <div className="group relative my-6 overflow-hidden rounded-2xl border border-border/80 bg-[oklch(0.12_0.02_250)] shadow-inner">
      <div className="flex items-center justify-between border-b border-border/60 px-4 py-2.5">
        <span className="rounded-md bg-rose-500/10 px-2 py-0.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-rose-400 ring-1 ring-rose-500/20">
          {language}
        </span>
        <button
          type="button"
