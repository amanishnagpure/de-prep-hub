...
**🎯 Interview answer:** PySpark is the Python API for Apache Spark. It allows us to process large amounts of data in a distributed manner using Python. Instead of processing the complete dataset on a single machine, Spark divides the data into partitions and processes them in parallel across multiple executors.
...
**🎯 Interview answer:** Apache Spark is the distributed data-processing engine, while PySpark is the Python API used to interact with Spark. Spark provides the actual execution engine, and PySpark allows Python developers to use Spark's capabilities.
...
**🎯 Interview answer:** We use PySpark when we need to process large datasets that cannot be efficiently handled on a single machine. It provides distributed processing, scalability, fault tolerance and optimization features, making it suitable for large-scale ETL pipelines.
...
**🎯 Interview answer:** The main advantages of PySpark are distributed processing, scalability, fault tolerance, lazy evaluation, query optimization, and support for DataFrames, SQL and streaming.
...
**🎯 Interview answer:** The main components of Spark architecture are the Driver, Executors and Cluster Manager. The Driver coordinates the application, Executors perform the actual tasks, and the Cluster Manager provides the required resources.
...
**🎯 Interview answer:** The Driver is the central coordinator of a Spark application. It runs the main application, creates the execution plan, divides the work into jobs, stages and tasks, and schedules those tasks on executors.
...
**🎯 Interview answer:** An Executor is a worker process that runs tasks assigned by the Driver. Executors process data partitions and can also store cached or persisted data.
...
**🎯 Interview answer:** A Cluster Manager is responsible for allocating resources such as CPU and memory to Spark applications and helping launch the required Spark processes.
...
**🎯 Interview answer:** SparkSession is the main entry point for modern Spark applications. It is used to create DataFrames, execute Spark SQL, read and write data, access configuration and interact with the Spark environment.
...
**🎯 Interview answer:** SparkContext is the lower-level entry point that connects a Spark application with the cluster and coordinates the execution of Spark jobs. In modern PySpark applications, we normally work with SparkSession, which provides access to the underlying SparkContext.
...
**🎯 Interview answer:** A PySpark DataFrame is a distributed collection of structured data organized into named columns. It is similar to a table in a database and provides a high-level API that Spark can optimize efficiently.
...
**🎯 Interview answer:** RDD is a low-level distributed data structure in Spark. It represents a collection of objects distributed across multiple partitions and provides fault tolerance through lineage.
...
**🎯 Interview answer:** RDD is a lower-level API that works with distributed objects, while DataFrame is a higher-level structured API with schema and named columns. DataFrames are generally preferred for ETL because Spark can optimize them using Catalyst and other execution optimizations.
...
**🎯 Interview answer:** DataFrames are preferred for most structured ETL workloads because they provide schema information and allow Spark's optimizer to optimize the query. They are generally easier to use and more efficient than manually working with RDDs.
...
**🎯 Interview answer:** Schema defines the structure of a DataFrame, including the column names and their data types. It tells Spark what kind of data each column contains.
...
**🎯 Interview answer:** PySpark provides data types for primitive values as well as complex nested structures. Common types include StringType, IntegerType, LongType, DoubleType, BooleanType, DateType, TimestampType, ArrayType, StructType and MapType.
...
**🎯 Interview answer:** We can define an explicit schema using StructType and StructField. In production pipelines, I prefer an explicit schema when the source structure is known because it provides predictable data types and avoids relying on schema inference.
...
**🎯 Interview answer:** `inferSchema` tells Spark to inspect the input data and infer the data types of columns instead of treating everything as strings for formats such as CSV.
...
**🎯 Interview answer:** Explicit schema is preferred in production because it makes the expected structure clear and predictable. It prevents unexpected type inference and helps detect bad or incompatible source data early.
...
**🎯 Interview answer:** We can use `spark.read.csv()` to read a CSV file. We can specify options such as whether the file has a header and whether Spark should infer the schema.
...
**🎯 Interview answer:** We can use `spark.read.json()` to read JSON data. Spark automatically creates a schema based on the JSON structure unless we provide an explicit schema.
...
**🎯 Interview answer:** We use `spark.read.parquet()` to read Parquet files. Parquet is a columnar format, so Spark can efficiently read only the required columns and can use file-level statistics for data skipping.
...
**🎯 Interview answer:** We use the DataFrame `write` API to write data to formats such as Parquet, JSON, CSV or Delta. We can also specify the write mode and partitioning.
...
**🎯 Interview answer:** PySpark provides append, overwrite, ignore and error-if-exists write modes. Append adds data, overwrite replaces existing output according to the write operation, ignore skips the write if the destination exists, and error fails when the destination already exists.
...
**🎯 Interview answer:** `select()` is used to choose required columns or create column expressions. It returns a new DataFrame and is a transformation.
...
**🎯 Interview answer:** `filter()` is used to keep only the records that satisfy a condition. It is a transformation and does not execute immediately because Spark uses lazy evaluation.
...
**🎯 Interview answer:** There is no major functional difference between `filter()` and `where()` in the DataFrame API. Both are used to filter rows based on a condition. `where()` is essentially an alias for `filter()`.
...
**🎯 Interview answer:** I use `withColumnRenamed()` to rename columns and `drop()` to remove columns. Both return a new DataFrame because Spark DataFrames are immutable.
...
**🎯 Interview answer:** `lit()` is used to create a literal constant value in a DataFrame expression, for example adding a fixed source-system or country value to every row.
...
**🎯 Interview answer:** In PySpark, I use `when()` and `otherwise()` to implement conditional logic, similar to CASE WHEN in SQL.
...
**🎯 Interview answer:** I handle nulls based on business requirements. I can remove null records using `dropna()`, replace them using `fillna()`, or use conditional logic when different columns require different handling.
...
**🎯 Interview answer:** `dropna()` removes rows containing nulls, while `fillna()` replaces null values with a specified value. The choice depends on the business requirement.
...
**🎯 Interview answer:** `orderBy()` and `sort()` provide the same DataFrame sorting functionality. For large datasets, sorting can be expensive because a global sort generally requires data redistribution and sorting across partitions.
...
**🎯 Interview answer:** `groupBy()` groups records based on one or more columns and is usually followed by an aggregation. Because records with the same grouping key may exist in different partitions, Spark generally performs a shuffle, making it a wide operation.
...
**🎯 Interview answer:** `countDistinct()` is used to count the number of unique values in a column.
...
**🎯 Interview answer:** `collect_list()` collects multiple values from different rows into a single array, and it keeps duplicates.
...
**🎯 Interview answer:** `collect_set()` collects values into an array but removes duplicates.
...
**🎯 Interview answer:** `explode()` converts each element of an array or map into a separate row.
...
**🎯 Interview answer:** PySpark provides functions and column notation to access and manipulate nested Array and Struct columns.
...
**🎯 Interview answer:** PySpark can automatically infer nested JSON structures or we can define an explicit schema.
...
**🎯 Interview answer:** Spark SQL allows us to use SQL queries to process Spark DataFrames and tables.
...
**🎯 Interview answer:** Common PySpark joins are:
...
**🎯 Interview answer:** An Inner Join returns only records that have matching keys in both DataFrames.
...
**🎯 Interview answer:** A Left Join returns all records from the left DataFrame and matching records from the right DataFrame.
...
**🎯 Interview answer:** A Left Semi Join returns records from the left DataFrame that have at least one matching record in the right DataFrame.
...
**🎯 Interview answer:** A Left Anti Join returns records from the left DataFrame that do NOT have a matching record in the right DataFrame.
...
**🎯 Interview answer:** A Cross Join creates every possible combination of rows between two DataFrames.
...
**🎯 Interview answer:** We can provide multiple join conditions using `&`.
...
**🎯 Interview answer:** A Window Function performs calculations across related rows without collapsing those rows into a single row.
...
**🎯 Interview answer:** `row_number()` assigns a unique sequential number to each row within a window partition.
...
**🎯 Interview answer:** `rank()` assigns the same rank to rows having the same ordering value, and leaves gaps after ties.
...
**🎯 Interview answer:** `dense_rank()` is similar to `rank()`, but does not leave gaps after ties.
...
**🎯 Interview answer:** `lag()` and `lead()` allow us to access a previous or next row's value within a window.
...
**🎯 Interview answer:** The main difference is that `groupBy()` reduces multiple rows into fewer rows, while a Window Function keeps the original rows.
...
**🎯 Interview answer:** * `count()` → counts records/non-null values.
...
**🎯 Interview answer:** A transformation creates a new DataFrame/RDD from an existing one without immediately executing the computation.
...
**🎯 Interview answer:** An action triggers the actual execution of the Spark job and usually returns a result or writes data.
...
**🎯 Interview answer:** Lazy evaluation means Spark does not execute transformations immediately.
...
**🎯 Interview answer:** DAG stands for Directed Acyclic Graph.
...
**🎯 Interview answer:** A Spark Job is created when an action is triggered.
...
**🎯 Interview answer:** A Stage is a group of tasks that can be executed together without requiring a shuffle boundary.
...
**🎯 Interview answer:** A Task is the smallest unit of execution sent to an executor.
...
**🎯 Interview answer:** Suppose we run:
...
**🎯 Interview answer:** A narrow transformation is one where each output partition depends on a relatively small/local set of input partitions, typically without requiring data to be shuffled across the cluster.
...
**🎯 Interview answer:** A wide transformation is one where output partitions may depend on data from multiple input partitions, so Spark needs to redistribute data, usually through a shuffle.
...
**🎯 Interview answer:** Shuffle is the process of redistributing data across partitions, usually based on a key, so that related records are brought together.
...
**🎯 Interview answer:** When we execute:
...
**🎯 Interview answer:** A partition is a logical chunk of data processed by Spark.
...
**🎯 Interview answer:** `repartition()` changes the number of partitions by performing a shuffle.
...
**🎯 Interview answer:** Both change the number of partitions, but the main difference is shuffle.
...
**🎯 Interview answer:** `spark.sql.shuffle.partitions` controls the default number of partitions created for many shuffle operations in Spark SQL/DataFrame workloads.
...
**🎯 Interview answer:** Data skew occurs when data is distributed unevenly across partitions, causing some partitions to contain much more data than others.
...
**🎯 Interview answer:** I would first check the Spark UI.
...
**🎯 Interview answer:** Common techniques are:
...
**🎯 Interview answer:** Salting is a technique used to distribute records belonging to a hot key across multiple partitions.
...
**🎯 Interview answer:** AQE allows Spark to modify parts of the execution plan at runtime using actual statistics collected during execution.
...
**🎯 Interview answer:** Both can help with skew, but they work differently.
...
**🎯 Interview answer:** A Broadcast Join sends a small DataFrame to the executors so that Spark can join it with a large DataFrame without shuffling the large DataFrame by the join key.
...
**🎯 Interview answer:** A Sort-Merge Join is a common distributed join strategy for joining large datasets.
...
**🎯 Interview answer:** | Broadcast Join                                | Sort-Merge Join                  |
| --------------------------------------------- | -------------------------------- |
...
**🎯 Interview answer:** Predicate pushdown means pushing filter conditions as close to the data source as possible so Spark reads less data.
...
**🎯 Interview answer:** Column pruning means Spark reads only the columns required by the query instead of reading every column.
...
**🎯 Interview answer:** Catalyst is Spark SQL's query optimization framework.
...
**🎯 Interview answer:** `explain()` is used to inspect the query execution plan of a DataFrame.
...
**🎯 Interview answer:** Caching stores a computed DataFrame/RDD so that it can be reused without recomputing the entire lineage.
...
**🎯 Interview answer:** This is one of the most important DE2 interview questions.
